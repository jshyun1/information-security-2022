...
def sdes_encrypt_ecb(text: bitarray, key: bitarray):
    ...
    for i in range(0,len(text),8):
        r += sdes(text[i:i+8], key,MODE_ENCRYPT)
    return r

def sdes_decrypt_ecb(ciphertext: bitarray, key: bitarray):
    r = bitarray()
    for i in range(0, len(ciphertext), 8):
        r += sdes(ciphertext[i:i + 8], key, MODE_DECRYPT)
    return r
...
def sdes_encrypt_cbc(text: bitarray, key: bitarray, iv:bitarray):
    ...
    if lenIv<8:
        tmp=[]
        for i in range(0,8-lenIv):
            tmp.append(0)
            iv = bitarray(tmp) +iv

    for i in range(0, len(text),8):
        if i==0:
            r += sdes(text[i:i + 8] ^ iv, key, MODE_ENCRYPT)

        else:
            r += sdes(text[i:i + 8] ^ (r[i-8:i]), key, MODE_ENCRYPT)
    return r
...
def sdes_decrypt_cbc(ciphertext: bitarray, key: bitarray, iv:bitarray):
    ...
    for i in range(0,lenCipher,8):
        if i ==0:
            r += sdes(ciphertext[i:i+8], key, MODE_DECRYPT)^iv
        else:
            r += sdes(ciphertext[i:i+8], key, MODE_DECRYPT)^ciphertext[i-8:i]
    return r
