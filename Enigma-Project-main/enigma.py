...
def pass_wheels(input, reverse = False):
    ...
    if reverse:
        reverse_wheel = reversed(SETTINGS["WHEELS"])
        for wh in reverse_wheel:
            wire = wh["wire"]
            input = wire[ord(input) - ord('A')]
...
def rotate_wheels():
    ...
    # deepcopy 를 이용해서 wheel의 마지막 원소를 추출해서 첫번째 자리에 넣는다.

    for wh in SETTINGS["WHEELS"]:
        str = wh["wire"][-1] + wh["wire"][1:]
        WHEELS[wh]["wire"] = str
